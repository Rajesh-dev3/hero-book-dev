...
const DetailOdds = ({visiblePrice,visible, selectionId, fancy, fancyId, fancyStatus, marketId, matchName, lay, item, value, price, height, border, fun, index, display, betPlaceHandler }) => {
  let oddsValue = value != 0 && value != null ? value : "--"
  let oddsPrice = price != 0  && price != null  ?  price : "--"
  return (
    <div className={`${visible && "invisible" } market-odd-box ${item}`} key={item} style={{ display: display, minHeight: height, borderLeft: border ? "1px solid #c7c8ca" : "", }}
      onClick={() => {
